...
import { Paper,Box,Typography,ButtonBase,Button,Fab ,Fragment,Avatar,makeStyles,Card,Select,ListItemIcon,MenuItem,InputLabel} from "@material-ui/core";
...
import {registerNpoClient,validateEfasheStartimesVendingTx,executeEfasheStartimesVendingTx,viewAgentFloatAccountTransactions,viewNpoAddresses} from '../../apis/UserController';
...
export default function StartimesServicePage() {
  ...
  const [showConfirmResponseDialog, setShowConfirmResponseDialog] = useState(false);
...
const [receiptMpostSystemMetadata,setReceiptMpostSystemMetadata]=useState("");
...
if(isNumber(subscriberNumber)){
        ...
        toast.error("The Subscription Amount Should be a set of valid amount numbers[0-9]");
        }

  }else{
    
    toast.error("Your Startimes Subscription No Should be a set of valid numbers[0-9]");
  }
...
const efasheTxValidatorRequestBody={
  ...
  "phoneNumber":subscriberNumber
    ...
    const response=await validateEfasheStartimesVendingTx(efasheTxValidatorRequestBody,context.userKey);
...
const efasheTxValidatorRequestBody={
  ...
  "agentCategory":context.agentCategory
    ...
    const response=await executeEfasheStartimesVendingTx(efasheTxValidatorRequestBody,context.userKey);

   
    if(response.responseCode==="200"){
      
      playAudio();
      toast.update(id, {render:response.responseDescription, type: "success", isLoading: false,closeButton: null});
        ...
        <div class="page-content-wrapper">
          ...
          <div class="discount-coupon-card-blue p-4 p-lg-5 dir-rtl">
              <div class="d-flex align-items-center">
                <div class="discountIcon"><img class="w-100" src="assets/img/bg-img/startimes-logo.jpg" alt=""/></div>
                <div class="text-content">
                 
                  <h4 class="text-white mb-1">Startimes Service</h4>
                  <p class="text-white mb-0">Buy Startimes Subscription with<span class="px-1 fw-bold"></span><b>Special Discounts.</b></p>
                </div>
              </div>
            </div>
          ...
          <div style={{backgroundColor:'#ffffff',borderRadius: 5, borderWidth:1,borderStyle:'solid'}}>
            <div class="row justify-content-center" >
              <div class="col-10 col-lg-8">
                ...
                <div class="form-group text-start mb-4"><span class="mb-2" style={{fontSize:24,color:'black'}}><b>Buy TV Subscription</b></span>
                  ...
                  <div class="form-group text-start mb-4"><span style={{color:'black',fontSize:16}}><b>Subscriber Number:</b><b style={{color:'red'}}>*</b></span>
                     
                     <input class="form-control" style={{backgroundColor:'white',color:'black',borderColor: "black",borderRadius: 10,borderWidth:1,borderStyle:'solid',fontSize:14}}  type="text"   onChange={(e) => setSubscriberNumber(e.target.value)} value={subscriberNumber} required/>
                   </div>
    
                    <div class="form-group text-start mb-4"><span style={{color:'black',fontSize:16}}><b>Subscription Amount:</b><b style={{color:'red'}}>*</b></span>
                     
                     <input class="form-control" style={{backgroundColor:'white',color:'black',borderColor: "black",borderRadius: 10,borderWidth:1,borderStyle:'solid',fontSize:14}}  type="text"   onChange={(e) => setSubscriptionAmount(e.target.value)} value={subscriptionAmount} required/>
                   </div>
            ...
            />
                    <PreviewStartimesServicePayment availTrxBalance={availTrxBalance} customerAccountNumber={customerAccountNumber} localStockMgt={localStockMgt} pdtId={pdtId} pdtName={pdtName} pdtStatusId={pdtStatusId} selectAmount={selectAmount} stock={stock} stockedPdts={stockedPdts} svcProviderName={svcProviderName} trxId={trxId} trxResult={trxResult} vendMax={vendMax} vendMin={vendMin} vendUnitId={vendUnitId} openstatus={showConfirmDialog} phoneNumber={value} amount={subscriptionAmount} closeClick={() => setShowConfirmDialog(!showConfirmDialog)} confirmClick={() => sendPaymentRequest()}/>                           
          ...
          <div class="weekly-best-seller-area py-3">
            <div class="container">
              <div class="section-heading d-flex align-items-center justify-content-between dir-rtl">
                ...
                <Link  class="btn p-0" onClick={viewFloatAccountInfo}>View All Transactions<i class="ms-1 fa-solid fa-arrow-right-long"></i></Link>
              </div>
              <div class="row g-2">
               {
                /*
            ...
            agentAccountTransactions.map((transaction,index)=>{
              ...
              if(context.agentCategory===null ||context.agentCategory==="Agent"){
                if(transaction.transactionType==="EFASHE Startimes Payment(Agent)"){
                      ...
                      <div class="d-flex align-items-center">
                        ...
                        <div class="product-description">
                         ...
                         <a class="product-title d-block" href="#">{transaction.description}</a>
                          ...
                          <div class="product-rating"><i class="fa-solid fa-star"></i>TX:{transaction.id}<span class="ms-1" style={{color:'red'}}><b>Amount Rwf:{((parseFloat(transaction.amount)/(29.30))+parseFloat(transaction.amount)).toFixed()}|</b></span><Link  to="/ddin-startimes-receipt" state={{transactionData:transaction,agentUsername:context.agentUsername}} >Receipt</Link></div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                if(transaction.transactionType==="EFASHE Startimes Payment(Corporate)"){
                      ...
                      <div class="d-flex align-items-center">
                        ...
                        <div class="product-description">
                         ...
                         <a class="product-title d-block" href="#">{transaction.description}</a>
                          ...
                          <div class="product-rating"><i class="fa-solid fa-star"></i>TX:{transaction.id}<span class="ms-1" style={{color:'red'}}><b>Amount Rwf:{((parseFloat(transaction.amount)/(29.30))+parseFloat(transaction.amount)).toFixed()}|</b></span><Link  to="/ddin-startimes-receipt" state={{transactionData:transaction,agentUsername:context.agentUsername}} >Preview Receipt</Link></div>
                        </div>
                      </div>
                    </div>
                  </div>
