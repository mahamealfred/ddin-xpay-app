...
import {registerNpoClient,viewEfasheTxStatus,validateEfasheElectricityVendingTx,executeEfasheElectricityVendingTx,viewAgentFloatAccountTransactions,viewNpoAddresses} from '../../apis/UserController';
...
const efasheTxValidatorRequestBody={
  ...
  "phoneNumber":meterNumber
    ...
    const response=await validateEfasheElectricityVendingTx(efasheTxValidatorRequestBody,context.userKey);
...
const efasheTxValidatorRequestBody={
  ...
  "agentCategory":context.agentCategory
    ...
    const response=await executeEfasheElectricityVendingTx(efasheTxValidatorRequestBody,context.userKey);

    if(response.responseCode==="200"){

      playAudio();
      toast.update(id, {render: response.responseDescription+",Voucher/Token:"+response.data.spVendInfo.voucher+",Units:"+response.data.spVendInfo.units+", for Rwf"+response.data.spVendInfo.unitsWorth, type: "success", isLoading: false,closeButton: null});
      setTokenAmount('');
      setMeterNumber('');
        ...
        <div class="page-content-wrapper">
          ...
          <div class="discount-coupon-card-blue p-4 p-lg-5 dir-rtl">
              <div class="d-flex align-items-center">
                <div class="discountIcon"><img class="w-100" src="assets/img/bg-img/eucllogo.PNG" alt=""/></div>
                <div class="text-content">
               
                  <h4 class="text-white mb-1">Electricity Service</h4>
                  <p class="text-white mb-0">Buy Electricity Token with<span class="px-1 fw-bold"></span><b>Special Discounts...</b></p>
                </div>
              </div>
            </div>
        ...
        <div class="container" >
          ...
          <div style={{backgroundColor:'#ffffff',borderRadius: 5, borderWidth:1,borderStyle:'solid'}}>
            <div class="row justify-content-center" >
              <div class="col-10 col-lg-8">
                ...
                <div class="form-group text-start mb-4"><span class="mb-2" style={{fontSize:24,color:'black'}}><b>Buy Electricity</b></span>
                  ...
                  <div class="form-group text-start mb-4"><span style={{color:'black',fontSize:16}}><b>Meter Number:</b><b style={{color:'red'}}>*</b></span>
                     
                     <input class="form-control" style={{backgroundColor:'white',color:'black',borderColor: "black",borderRadius: 10,borderWidth:1,borderStyle:'solid',fontSize:14}}  type="text"   onChange={(e) => setMeterNumber(e.target.value)} value={meterNumber} required/>
                   </div>

                    <div class="form-group text-start mb-4"><span style={{color:'black',fontSize:16}}><b>Token Amount:</b><b style={{color:'red'}}>*</b></span>
                     
                     <input class="form-control" style={{backgroundColor:'white',color:'black',borderColor: "black",borderRadius: 10,borderWidth:1,borderStyle:'solid',fontSize:14}}  type="text"   onChange={(e) => setTokenAmount(e.target.value)} value={tokenAmount} required/>
                   </div>
            ...
            />
                    <PreviewElectricityServicePayment availTrxBalance={availTrxBalance} customerAccountNumber={customerAccountNumber} localStockMgt={localStockMgt} pdtId={pdtId} pdtName={pdtName} pdtStatusId={pdtStatusId} selectAmount={selectAmount} stock={stock} stockedPdts={stockedPdts} svcProviderName={svcProviderName} trxId={trxId} trxResult={trxResult} vendMax={vendMax} vendMin={vendMin} vendUnitId={vendUnitId} openstatus={showConfirmDialog} phoneNumber={value} amount={tokenAmount} closeClick={() => setShowConfirmDialog(!showConfirmDialog)} confirmClick={() => sendPaymentRequest()}/>                           
          ...
          <div class="weekly-best-seller-area py-3">
            ...
            agentAccountTransactions.map((transaction,index)=>{
              ...
              if(context.agentCategory===null ||context.agentCategory==="Agent"){
                if(transaction.transactionType==="EFASHE Electricity Payment(Agent)"){
                      ...
                      <div class="d-flex align-items-center">
                        ...
                        <div class="product-description">
                         ...
                         <a class="product-title d-block" href="#">{transaction.description}</a>
                          ...
                          <div class="product-rating"><i class="fa-solid fa-star"></i>TX:{transaction.id}<span class="ms-1" style={{color:'red'}}><b>Amount Rwf:{((parseFloat(transaction.amount)*(0.02))+parseFloat(transaction.amount)).toFixed()}|</b></span><Link  to="/ddin-electricity-receipt" state={{transactionData:transaction,agentUsername:context.agentUsername}} >Receipt</Link></div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                if(transaction.transactionType==="EFASHE Electricity Payment(Corporate)"){
                      ...
                      <div class="d-flex align-items-center">
                        ...
                        <div class="product-description">
                         ...
                         <a class="product-title d-block" href="#">{transaction.description}</a>
                          ...
                          <div class="product-rating"><i class="fa-solid fa-star"></i>TX:{transaction.id}<span class="ms-1" style={{color:'red'}}><b>Amount Rwf:{((parseFloat(transaction.amount)*(0.02))+parseFloat(transaction.amount)).toFixed()}|</b></span><Link  to="/ddin-electricity-receipt" state={{transactionData:transaction,agentUsername:context.agentUsername}} >Preview Receipt</Link></div>
                        </div>
                      </div>
                    </div>
                  </div>
